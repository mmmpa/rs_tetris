...
pub trait Minos {
    fn right(self) -> (Mino, Offset);
    fn left(self) -> (Mino, Offset);
    fn cells(&self) -> &[&[u8]];
}
...
macro_rules! mino {
    ( $now:tt, $right:tt, $right_x:tt, $right_y:tt, $left:tt, $left_x:tt, $left_y:tt, $cells:expr ) => {
        impl Minos for $now {
            fn right(self) -> (Mino, Offset) {
                next_mino(Mino::$right($right), Length::$right_x, Length::$right_y)
            }

            fn left(self) -> (Mino, Offset) {
                next_mino(Mino::$left($left), Length::$left_x, Length::$left_y)
            }
...
fn next_mino(next: Mino, x: Length, y: Length) -> NextMino {
    (next, Offset { x, y })
}
